...
transformed parameters {
  ...
  matrix[Nv,Nm] PPM_n_standardise;
  matrix[Nv,Nm] PPM_nn_standardise;
  ...
  PPM_n_standardise = (PPM_n - mean(PPM_n))/sd(PPM_n);
  PPM_nn_standardise = (PPM_nn - mean(PPM_nn))/sd(PPM_nn);

  // Add power means to design matrix
  Xa[,1:K] = Xc;
  Xa[,K+1] = to_vector(PPM_n_standardise) - mean(PPM_n_standardise);
  Xa[,K+2] = to_vector(PPM_nn_standardise) - mean(PPM_nn_standardise);
  means_Xa[K] = mean(PPM_n_standardise);
  means_Xa[K+1] = mean(PPM_nn_standardise);
